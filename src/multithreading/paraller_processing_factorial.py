...
# Вариант Б: Использование multiprocessing.Pool
...
def compare_performance(data):
    ...
    start = time.time()
    single_thread_result = [process_number(num) for num in data]
    results['Single Thread'] = time.time() - start

    # Вариант А с использованием пула потоков
    start = time.time()
    ...
    # Вариант В с созданием отдельных процессов с очередями
    start = time.time()
...
if __name__ == "__main__":
    n = 1000000  # Количество случайных чисел
